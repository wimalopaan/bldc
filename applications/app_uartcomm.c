...
#ifdef HW_USE_HALF_DUPLEX_PPM
	uart_cfg[0].cr3 |= USART_CR3_HDSEL;
#endif
...
#ifdef HW_USE_HALF_DUPLEX_PPM
static volatile systime_t last_send_time = 0;
#endif
...
#ifdef HW_USE_HALF_DUPLEX_PPM
	last_send_time = chVTGetSystemTimeX();
	if (port_number == 0) {
...
#ifdef HW_USE_HALF_DUPLEX_PPM
				if (port_number == 0) {
					const systime_t dt = ST2MS(chVTTimeElapsedSinceX(last_send_time));
					if (dt < 10) {
						continue;
					}
					else {
